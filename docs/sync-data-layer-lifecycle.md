...
| `processing_jobs`                                           | The actual unit claimed by a worker.                                                               | Belongs to one source and principal; carries attempt, heartbeat, retry, and per-job progress data. |
...
The write logic lives in `SourceSyncJobRepositoryLive`: `createOrReuseJob` inserts the pending row, and later methods claim it, heartbeat it, retry it, complete it, or fail it ([repository](../packages/persistence/src/layers/SourceSyncJobRepositoryLive.ts)).
...
## Chapter 3: the worker claims the job from Postgres

There is no message broker. The pending `processing_jobs` row is the whole hand-off: the API writes it and returns, stamping `queued_at` at creation. The worker runs a poll loop that lists claimable jobs - pending, prerequisites met, and any retry delay passed - and hands each id to the executor ([poll loop](../apps/worker/src/layers/WorkerSourceSyncPollerLive.ts), [claimable listing](../packages/persistence/src/layers/SourceSyncJobRepositoryLive.ts)).

`SourceSyncJobExecutor.execute` claims the row with `FOR UPDATE SKIP LOCKED` and atomically changes it from `pending` to `processing`, recording `worker_id`, `started_at`, `heartbeat_at`, and the attempt count. A concurrent claim by another worker sees no row and reports a typed conflict, so several worker processes can poll the same table safely ([claim write](../packages/persistence/src/layers/SourceSyncJobRepositoryLive.ts)).

Crash recovery is a periodic sweep in the same worker: a `processing` job whose heartbeat is older than the stale cutoff is failed, its owed follow-up replay is created, and dependent jobs are cascade-failed. One sweep also runs at worker boot before the first dispatch.
...
- A retryable provider failure writes the message to `source_sync_state.last_error_message`, returns `processing_jobs` to `pending`, clears the worker claim, and records an exponential `next_retry_at`. The worker poll loop skips the job until that time passes, then claims it again; claiming counts the attempt, and the retry stops once `attempt_count` reaches `max_attempts`.
...
- Heartbeats on `processing_jobs` let the stale-job sweep identify work abandoned by a dead worker.
...
| `processing_jobs`                    | `SourceSyncJobRepositoryLive`; progress fields also by `SourceSyncStateRepositoryLive`                                                           | Creates and claims work, stores attempts, retry timing, heartbeat, errors, checkpoints, progress, and completion. |
...
3. [`SourceSyncServiceLive.ts`](../packages/sync-engine/src/layers/SourceSyncServiceLive.ts) — active-job reuse and stale recovery.
...
5. [`WorkerSourceSyncPollerLive.ts`](../apps/worker/src/layers/WorkerSourceSyncPollerLive.ts) — the poll loop, claims, and the stale-job sweep.
