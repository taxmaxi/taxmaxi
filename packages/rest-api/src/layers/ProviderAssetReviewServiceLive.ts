/**
 * ProviderAssetReviewServiceLive - CoinGecko-backed provider asset review.
 *
...
const makeBadRequest = (message: string) => new ProviderAssetReviewBadRequestError({ message })

export const optionalCandidateResolution = <A>(
  effect: Effect.Effect<A, ProviderAssetReviewBadRequestError | ProviderAssetReviewProviderError>
) =>
  effect.pipe(
    Effect.map((candidate) => ({ _tag: "Candidate" as const, candidate })),
    Effect.catchTag("ProviderAssetReviewBadRequestError", (error) =>
      Effect.succeed({
...
}): Effect.Effect<void, ProviderAssetReviewBadRequestError> => {
...
}): Effect.Effect<CoinGeckoSearchCoin, ProviderAssetReviewBadRequestError> => {
...
const make = Effect.gen(function* () {
  ...
  const mapCoinGeckoError = (error: { readonly message: string }) =>
    new ProviderAssetReviewProviderError({ message: error.message })
  ...
  const loadPendingReview = (providerAssetRowId: string) =>
    Effect.gen(function* () {
      const review = yield* providerAssetReviewRepository
  ...
  const loadReviewAfterDecision = (providerAssetRowId: string) =>
    providerAssetReviewRepository.findProviderAssetReviewById({ providerAssetRowId }).pipe(
      Effect.mapError(
        () =>
          new ProviderAssetReviewInternalError({
  ...
  const canonicalizeProviderAssetFromCoinGecko: ProviderAssetReviewServiceShape["canonicalizeProviderAssetFromCoinGecko"] =
  ...
  const mapProviderAssetToExisting: ProviderAssetReviewServiceShape["mapProviderAssetToExisting"] =
    ({ providerAssetRowId, canonicalAssetId, reviewerNotes, reviewedBy }) =>
      Effect.gen(function* () {
        ...
        if (Option.isNone(target)) {
          return yield* Effect.fail(
            new ProviderAssetReviewBadRequestError({
  ...
  const approveProviderAssetAsFiat: ProviderAssetReviewServiceShape["approveProviderAssetAsFiat"] =
    ({ providerAssetRowId, reviewedBy }) =>
      Effect.gen(function* () {
        ...
        const decision = yield* providerAssetReviewRepository
  ...
  const rejectProviderAsset: ProviderAssetReviewServiceShape["rejectProviderAsset"] = ({
    providerAssetRowId,
    rejectionReason,
    reviewedBy,
  }) =>
      ...
      .pipe(
        ...
        Effect.flatMap(
          Option.match({
            onNone: () =>
              Effect.fail(
                new ProviderAssetReviewNotFoundError({
  ...
  const getProviderAssetReplay: ProviderAssetReviewServiceShape["getProviderAssetReplay"] = ({
    providerAssetRowId,
    sourceId,
    jobId,
  }) =>
        ...
        .pipe(
          Effect.mapError((error) =>
            error._tag === "SourceSyncJobNotFoundError"
              ? new ProviderAssetReviewNotFoundError({ message: "Replay job was not found." })
              : new ProviderAssetReviewInternalError({
  ...
  const retryProviderAssetReplay: ProviderAssetReviewServiceShape["retryProviderAssetReplay"] = ({
    providerAssetRowId,
    sourceId,
    jobId,
  }) =>
    Effect.gen(function* () {
      const source = yield* loadReplaySource({ providerAssetRowId, sourceId, jobId })
      const replay = yield* sourceSyncService
      ...
      const rebound = yield* providerAssetReviewRepository
        .replaceProviderAssetReplayJob({
          providerAssetRowId,
          sourceId,
          previousJobId: jobId,
          nextJobId: replay.jobId,
        })
        .pipe(
          Effect.mapError(
            () =>
              new ProviderAssetReviewInternalError({
                message: "Failed to bind the retried replay job.",
              })
          )
        )
      ...
      if (!rebound) {
        return yield* Effect.fail(
          new ProviderAssetReviewConflictError({
            message: "The replay job changed before the retry could be recorded.",
          })
        )
      }
  ...
  } satisfies ProviderAssetReviewServiceShape)
})
