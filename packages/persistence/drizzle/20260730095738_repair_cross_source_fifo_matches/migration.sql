...
USING "_invalid_cross_source_disposal_matches"
WHERE "disposal_matches"."id" = "_invalid_cross_source_disposal_matches"."match_id";--> statement-breakpoint

DELETE FROM "inventory_movement_allocations"
USING "_invalid_cross_source_movement_allocations"
WHERE "inventory_movement_allocations"."id" = "_invalid_cross_source_movement_allocations"."allocation_id";--> statement-breakpoint
...
SELECT
	...
	"transaction_type",
	"transaction_type",
	'Review required because an earlier FIFO allocation consumed inventory from another source. The invalid allocation was removed and the source was marked for deterministic re-normalization.',
	'fifo_inventory',
	true,
	NULL,
	now(),
