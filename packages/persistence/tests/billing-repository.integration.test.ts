...
const REVERSAL_EVENT_AT = DateTime.toDateUtc(DateTime.makeUnsafe("2026-08-13T12:00:00.000Z"))
...
describe("BillingRepositoryLive", () => {
  beforeEach(() =>
    Effect.runPromise(
      Effect.gen(function* () {
        ...
        yield* Effect.promise(() => seedBillingAccount())
      })
    )
  )

  it.effect("keeps one zero-due annual allowance across webhook retries", () =>
    Effect.gen(function* () {
      const expiresAt = DateTime.toDateUtc(DateTime.makeUnsafe("2027-08-14T00:00:00.000Z"))
      const grant = {
        userId: TEST_USER_ID,
        amount: 10_000,
  ...
  it.effect("moves a payment reversal to the latest proportional refund amount", () =>
    Effect.gen(function* () {
        ...
        runRepository(
          Effect.gen(function* () {
            ...
            yield* repository.setPaymentCreditReversal({
              ...
              eventCreatedAt: DateTime.toDateUtc(
                DateTime.makeUnsafe(REVERSAL_EVENT_AT.getTime() + 1_000)
              ),
              monotonic: true,
              terminal: false,
            })
  ...
  it.effect("rounds once after combining separate partial refunds", () =>
    Effect.gen(function* () {
      const available = yield* Effect.promise(() =>
        runRepository(
          Effect.gen(function* () {
            ...
            yield* repository.setPaymentCreditReversal({
              ...
              eventCreatedAt: DateTime.toDateUtc(
                DateTime.makeUnsafe(REVERSAL_EVENT_AT.getTime() + 2_000)
              ),
              stripeInvoiceId: "in_small_refunds",
              monotonic: true,
              terminal: true,
  ...
  it.effect("reverses the credit linked to a one-cent annual payment", () =>
    Effect.gen(function* () {
      const expiresAt = DateTime.toDateUtc(DateTime.makeUnsafe("2027-08-14T00:00:00.000Z"))
      const available = yield* Effect.promise(() =>
        runRepository(
  ...
  it.effect.each([
    ...
    {
      firstExpiresAt: DateTime.toDateUtc(DateTime.makeUnsafe("2027-08-14T00:00:00.000Z")),
      name: "across expiry buckets",
      secondExpiresAt: null,
    },
  ])("rounds a shared-payment reversal once $name", ({ firstExpiresAt, secondExpiresAt }) =>
    Effect.gen(function* () {
      const result = yield* Effect.promise(() =>
        runRepository(
          Effect.gen(function* () {
      ...
      const adjustments = yield* Effect.promise(() =>
        context.runPg(
          Effect.gen(function* () {
            const db = yield* drizzle
            return yield* db
      ...
      expect(adjustments.some(({ reference }) => reference.includes("in_rounding_first"))).toBe(
        true
      )
    })
  )

  it.effect("moves a shared-payment refund from payment-wide to its credited invoice", () =>
    Effect.gen(function* () {
      const result = yield* Effect.promise(() =>
        runRepository(
          Effect.gen(function* () {
  ...
  it.effect("applies a reversal that arrives before its payment grant", () =>
    Effect.gen(function* () {
      const result = yield* Effect.promise(() =>
        runRepository(
          Effect.gen(function* () {
  ...
  it.effect("serializes concurrent grants and reversals for the same payment", () =>
    Effect.gen(function* () {
      ...
      const available = yield* Effect.promise(() =>
        runRepository(
          Effect.flatMap(BillingRepository, (repository) =>
            repository.availableCredits(TEST_USER_ID)
          )
        )
      )
  ...
  it.effect("recalculates refund debt across split annual payments", () =>
    Effect.gen(function* () {
      const expiresAt = DateTime.toDateUtc(DateTime.addDuration(yield* DateTime.now, "1 hour"))
      const result = yield* Effect.promise(() =>
        runRepository(
          Effect.gen(function* () {
            const repository = yield* BillingRepository
  ...
  it.effect("reverses every annual period funded by the same payment", () =>
    Effect.gen(function* () {
      const firstExpiry = DateTime.toDateUtc(DateTime.addDuration(yield* DateTime.now, "1 hour"))
      const secondExpiry = DateTime.toDateUtc(DateTime.addDuration(yield* DateTime.now, "2 hours"))
      const available = yield* Effect.promise(() =>
        runRepository(
          Effect.gen(function* () {
            const repository = yield* BillingRepository
  ...
  it.effect("moves refund debt when later grants change the expiry bucket", () =>
    Effect.gen(function* () {
      const expiresAt = DateTime.toDateUtc(DateTime.addDuration(yield* DateTime.now, "1 hour"))
      yield* Effect.promise(() =>
        runRepository(
  ...
  it.effect("does not let an older reversal event reduce a newer cumulative refund", () =>
    Effect.gen(function* () {
      const available = yield* Effect.promise(() =>
        runRepository(
          Effect.gen(function* () {
  ...
  it.effect("reserves one annual Checkout generation for concurrent requests", () =>
    Effect.gen(function* () {
        ...
        runRepository(
          Effect.gen(function* () {
            ...
            return yield* Effect.all(
              [
                repository.reserveAnnualCheckout({
                  userId: TEST_USER_ID,
  ...
  it.effect("advances the annual Checkout generation after its reservation expires", () =>
    Effect.gen(function* () {
        ...
        context.runPg(
          Effect.gen(function* () {
            ...
            yield* db
              ...
              .set({
                annualCheckoutExpiresAt: DateTime.toDateUtc(
  ...
  it.effect("pins the annual price for the lifetime of a Checkout reservation", () =>
    Effect.gen(function* () {
      const reservations = yield* Effect.promise(() =>
        runRepository(
          Effect.gen(function* () {
  ...
  it.effect("clears only the current failed annual Checkout reservation", () =>
    Effect.gen(function* () {
      const result = yield* Effect.promise(() =>
        runRepository(
          Effect.gen(function* () {
  ...
  it.effect("invalidates the Checkout reservation that produced a terminal subscription", () =>
    Effect.gen(function* () {
      const result = yield* Effect.promise(() =>
        runRepository(
          Effect.gen(function* () {
  ...
  it.effect("keeps a newer Checkout reservation when an older terminal subscription arrives", () =>
    Effect.gen(function* () {
      const result = yield* Effect.promise(() =>
        runRepository(
          Effect.gen(function* () {
  ...
  it.effect("uses new credits to cover refunded usage debt", () =>
    Effect.gen(function* () {
      const expiresAt = DateTime.toDateUtc(DateTime.addDuration(yield* DateTime.now, "1 hour"))
      const available = yield* Effect.promise(() =>
        runRepository(
  ...
  it.effect("does not let an older subscription event overwrite newer state", () =>
    Effect.gen(function* () {
      const newestEventAt = DateTime.toDateUtc(DateTime.makeUnsafe("2026-08-13T12:00:02.000Z"))
      const staleEventAt = DateTime.toDateUtc(DateTime.makeUnsafe("2026-08-13T12:00:01.000Z"))
        ...
        runRepository(
          Effect.gen(function* () {
            ...
            const acceptedNewest = yield* repository.saveSubscription({
              ...
              status: "active",
              currentPeriodEnd: DateTime.toDateUtc(DateTime.makeUnsafe("2027-08-13T12:00:00.000Z")),
              cancelAtPeriodEnd: false,
  ...
  it.effect("accepts a newer subscription event after a delayed older event reserves later", () =>
    Effect.gen(function* () {
      const baselineEventAt = DateTime.toDateUtc(DateTime.makeUnsafe("2026-08-13T12:00:02.000Z"))
      const currentEventAt = DateTime.toDateUtc(DateTime.makeUnsafe("2026-08-13T12:00:03.000Z"))
      const delayedEventAt = DateTime.toDateUtc(DateTime.makeUnsafe("2026-08-13T12:00:01.000Z"))
        ...
        runRepository(
          Effect.gen(function* () {
            ...
            const acceptedCurrent = yield* repository.saveSubscription({
              ...
              stripeSubscriptionId: "sub_ordered",
              status: "active",
              currentPeriodEnd: DateTime.toDateUtc(DateTime.makeUnsafe("2027-08-13T12:00:03.000Z")),
              cancelAtPeriodEnd: false,
              eventCreatedAt: currentEventAt,
  ...
  it.effect("accepts a newer subscription clear after a delayed older event reserves later", () =>
    Effect.gen(function* () {
      const baselineEventAt = DateTime.toDateUtc(DateTime.makeUnsafe("2026-08-13T12:00:02.000Z"))
      const currentEventAt = DateTime.toDateUtc(DateTime.makeUnsafe("2026-08-13T12:00:03.000Z"))
      const delayedEventAt = DateTime.toDateUtc(DateTime.makeUnsafe("2026-08-13T12:00:01.000Z"))
        ...
        runRepository(
          Effect.gen(function* () {
            ...
            yield* repository.saveSubscription({
              ...
              status: "active",
              currentPeriodEnd: DateTime.toDateUtc(DateTime.makeUnsafe("2027-08-13T12:00:02.000Z")),
              cancelAtPeriodEnd: false,
              eventCreatedAt: baselineEventAt,
              syncGeneration: 0,
            })
  ...
  it.effect("rejects a late subscription fetch after a newer sync reservation", () =>
    Effect.gen(function* () {
      const result = yield* Effect.promise(() =>
        runRepository(
          Effect.gen(function* () {
            const repository = yield* BillingRepository
            const eventCreatedAt = DateTime.toDateUtc(
              DateTime.makeUnsafe("2026-08-13T12:00:00.000Z")
            )
  ...
  it.effect("clears a tracked subscription only under the winning sync generation", () =>
    Effect.gen(function* () {
      const result = yield* Effect.promise(() =>
        runRepository(
          Effect.gen(function* () {
            ...
            yield* repository.saveSubscription({
              ...
              currentPeriodEnd: DateTime.toDateUtc(DateTime.makeUnsafe("2027-08-13T12:00:00.000Z")),
              cancelAtPeriodEnd: false,
              eventCreatedAt: DateTime.toDateUtc(DateTime.makeUnsafe("2026-08-13T12:00:00.000Z")),
              syncGeneration: 0,
            })
            const input = {
              stripeCustomerId: STRIPE_CUSTOMER_ID,
              stripeSubscriptionId: "sub_moved_off_annual",
              eventCreatedAt: DateTime.toDateUtc(DateTime.makeUnsafe("2026-08-13T12:00:01.000Z")),
            } as const
