...
describe("SyncEngineSourceRepositoryLive", () => {
  ...
  it.effect("loads only sources owned by the requesting user", () =>
    Effect.gen(function* () {
      const ownedSource = yield* Effect.promise(() =>
        runRepository(
  ...
  it.effect("loads wallet address context for onchain sources", () =>
    Effect.gen(function* () {
  ...
  it.effect("does not hydrate wallet addresses owned by another principal", () =>
    Effect.gen(function* () {
        ...
        runRepository(
          Effect.flatMap(SourceRepository, (repository) =>
            Effect.all({
              ...
              listed: repository.listPrincipalSourceSyncContexts({
                principalId: TEST_PRINCIPAL_ID,
              }),
            })
          )
