...
const seedPendingApprovalAsset = (
  suffix: string,
    ...
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        runPg(
          Effect.gen(function* () {
            const db = yield* drizzle
            const timestamp = DateTime.toDateUtc(DateTime.makeUnsafe("2025-04-20T13:00:00.000Z"))
...
describe("ProviderAssetRepositoryLive", () => {
  describe("current schema", () => {
    ...
    it.effect("upserts provider assets by stable provider asset id and resolves mappings", () =>
      Effect.gen(function* () {
        const firstUpsertCount = yield* Effect.promise(() =>
          runRepository(
        ...
        const secondUpsertCount = yield* Effect.promise(() =>
          runRepository(
            Effect.flatMap(ProviderAssetRepository, (repository) =>
              repository.upsertProviderAssets({
                providerKey: "coinbase",
        ...
        const mappingUpsertCount = yield* Effect.promise(() =>
          runRepository(
            Effect.flatMap(ProviderAssetRepository, (repository) =>
        ...
        const providerAssetRows = yield* Effect.promise(() =>
          runPg(
            Effect.gen(function* () {
              const db = yield* drizzle
              return yield* db
    ...
    it.effect("bumps evidence revision only when stored observation facts change", () =>
      Effect.gen(function* () {
        const upsertBtc = ({
          name,
          payload,
        ...
        const afterUnchanged = yield* Effect.promise(() =>
          runPg(
            Effect.gen(function* () {
              const db = yield* drizzle
              const [row] = yield* db
        ...
        const afterChanged = yield* Effect.promise(() =>
          runPg(
            Effect.gen(function* () {
              const db = yield* drizzle
              const [row] = yield* db
    ...
    it.effect("does not downgrade a settled mapping when a late pending write arrives", () =>
      Effect.gen(function* () {
        const providerAsset = yield* Effect.promise(() =>
          seedPendingApprovalAsset("late-pending", {
        ...
        yield* Effect.promise(() => writeMapping("approved"))
        yield* Effect.promise(() => writeMapping("pending_review"))
    ...
    it.effect("locks provider rows before racing trusted seeds with manual mapping writes", () =>
      Effect.gen(function* () {
        const providerAsset = yield* Effect.promise(() =>
          runRepository(
            Effect.gen(function* () {
        ...
        const providerLocked = yield* Deferred.make<void>()
        const releaseManualMapping = yield* Deferred.make<void>()
        ...
        yield* Deferred.await(providerLocked)
        ...
        yield* Deferred.succeed(releaseManualMapping, undefined)
        yield* Effect.promise(() => manualMapping)
        expect(yield* Effect.promise(() => trustedSeed)).toBe(0)
    ...
    it.effect("reevaluates later evidence without replacing a trusted mapping conclusion", () =>
      Effect.gen(function* () {
        const providerAsset = yield* Effect.promise(() =>
          runRepository(
            Effect.gen(function* () {
    ...
    it.effect("reevaluates a settled fiat mapping only when the provider reclassifies it", () =>
      Effect.gen(function* () {
        ...
        yield* Effect.promise(() =>
          upsertFiatEntry({ name: "Polish Zloty", providerType: "fiat", version: 1 })
        )
        const providerAsset = yield* Effect.promise(() =>
          runRepository(
            Effect.gen(function* () {
        ...
        yield* Effect.promise(() =>
          upsertFiatEntry({ name: "Zloty", providerType: "fiat", version: 2 })
        )
        expect(yield* Effect.promise(() => loadJobs())).toEqual([])

        yield* Effect.promise(() =>
          upsertFiatEntry({ name: "Zloty", providerType: "crypto", version: 3 })
        )
        expect(yield* Effect.promise(() => loadJobs())).toEqual([
    ...
    it.effect("schedules a fresh evaluation when the resolution policy revision changes", () =>
      Effect.gen(function* () {
        const upsertEntry = () =>
          runRepository(
            Effect.flatMap(ProviderAssetRepository, (repository) =>
        ...
        yield* Effect.promise(() => upsertEntry())
        const providerAsset = yield* Effect.promise(() =>
          runRepository(
            Effect.gen(function* () {
              const repository = yield* ProviderAssetRepository
    ...
    it.effect("approves once and attaches replay to an active job under concurrent retries", () =>
      Effect.gen(function* () {
        const providerAsset = yield* Effect.promise(() =>
          runRepository(
            Effect.gen(function* () {
        ...
        const pendingPolicyEvaluationId = yield* Effect.promise(() =>
          runPg(
            Effect.gen(function* () {
              const db = yield* drizzle
              const timestamp = DateTime.toDateUtc(DateTime.makeUnsafe("2025-04-20T12:00:00.000Z"))
              const [transaction] = yield* db
                .insert(schema.transactions)
                .values({
                  sourceId: TEST_SOURCE_ID,
        ...
        const results = yield* Effect.promise(() => Promise.all([approve(), approve()]))
        const state = yield* Effect.promise(() =>
          runPg(
            Effect.gen(function* () {
              const db = yield* drizzle
    ...
    it.effect("rejects approval when no current conclusion owns the target", () =>
      Effect.gen(function* () {
        const providerAsset = yield* Effect.promise(() =>
          seedPendingApprovalAsset("no-active-owner")
        )
    ...
    it.effect("rejects inconsistent human conclusions without writing approval state", () =>
      Effect.gen(function* () {
        const providerAsset = yield* Effect.promise(() =>
          seedPendingApprovalAsset("invalid-human-conclusion")
        )
    ...
    it.effect("rejects approval when the current conclusion owns a different target", () =>
      Effect.gen(function* () {
        const providerAsset = yield* Effect.promise(() =>
          seedPendingApprovalAsset("different-active-owner")
        )
    ...
    it.effect("rejects chainless approval when the current conclusion owns a different asset", () =>
      Effect.gen(function* () {
        const providerAsset = yield* Effect.promise(() =>
          seedPendingApprovalAsset("different-chainless-asset")
        )
        const currentConclusionId = yield* Effect.promise(() =>
          runPg(
            Effect.gen(function* () {
    ...
    it.effect("replays provider sources even when the asset has no provider-transfer row", () =>
      Effect.gen(function* () {
        const providerAsset = yield* Effect.promise(() =>
          seedPendingApprovalAsset("non-transfer-use", {
            withProviderTransfer: false,
          })
        )
    ...
    it.effect("backfills an unresolved asset from an appended provider-asset review segment", () =>
      Effect.gen(function* () {
        const providerAsset = yield* Effect.promise(() =>
          seedPendingApprovalAsset("legacy-appended-review", {
            withProviderTransfer: false,
          })
        )
        yield* Effect.promise(() =>
            ...
            Effect.gen(function* () {
              ...
              const [transaction] = yield* db
                ...
                .values({
                  ...
                  timestamp: DateTime.toDateUtc(DateTime.makeUnsafe("2025-04-20T13:00:00.000Z")),
                  providerTransactionType: "send",
                  providerStatus: "completed",
                  principalId: TEST_PRINCIPAL_ID,
                })
    ...
    it.effect("tracks policy-backed replay when a source use is recorded after approval", () =>
      Effect.gen(function* () {
        const providerAsset = yield* Effect.promise(() =>
          seedPendingApprovalAsset("approval-before-source-use", {
            withProviderTransfer: false,
          })
        )
        const pendingPolicyEvaluationId = yield* Effect.promise(() =>
        ...
        const recordedAgain = yield* Effect.promise(() =>
          runRepository(
            Effect.flatMap(ProviderAssetRepository, (repository) =>
              repository.recordProviderAssetSourceUses({
                sourceId: TEST_SOURCE_ID,
    ...
    it.effect("requests replay when a source use is recorded after exclusion", () =>
      Effect.gen(function* () {
        const providerAsset = yield* Effect.promise(() =>
          seedPendingApprovalAsset("exclusion-before-source-use", {
            withProviderTransfer: false,
          })
        )
    ...
    it.effect("tracks automatic exclusion replay work against the active decision", () =>
      Effect.gen(function* () {
        const providerAsset = yield* Effect.promise(() =>
          seedPendingApprovalAsset("tracked-automatic-exclusion")
        )
              ...
              return yield* db
                ...
                .innerJoin(
                  ...
                  eq(
                    ...
                    schema.assetDecisionRematerializations.processingJobId
                  )
                )
            })
          )
    ...
    it.effect("rolls back the exclusion decision when the mapping transition fails", () =>
      Effect.gen(function* () {
        const providerAsset = yield* Effect.promise(() =>
          seedPendingApprovalAsset("atomic-exclusion-rollback", {
            withProviderTransfer: false,
          })
        )
        yield* Effect.promise(() =>
    ...
    it.effect("rejects a chainless exclusion reversal outside the revision-bound review flow", () =>
      Effect.gen(function* () {
        const providerAssetId = "btc-approval-manual-exclusion-reversal"
        const providerAsset = yield* Effect.promise(() =>
          seedPendingApprovalAsset("manual-exclusion-reversal")
        )
        yield* Effect.promise(() =>
          runRepository(
            Effect.flatMap(ProviderAssetRepository, (repository) =>
        ...
        const reviewedEvidenceIds = yield* Effect.promise(() =>
          runPg(
            Effect.gen(function* () {
              const db = yield* drizzle
              ...
              const [ownedEvidence] = yield* db
                ...
                .values({
                  ...
                  claimKind: "spam",
                  retrievedAt: DateTime.toDateUtc(DateTime.makeUnsafe("2026-08-21T12:01:00.000Z")),
                  evidenceRevision: 1,
              ...
              const [linkedEvidence] = yield* db
                ...
                .values({
                  ...
                  claimKind: "spam",
                  retrievedAt: DateTime.toDateUtc(DateTime.makeUnsafe("2026-08-21T12:02:00.000Z")),
                  evidenceRevision: 1,
                  decodedClaim: { verdict: "excluded" },
        ...
        const revisedProviderAsset = yield* Effect.promise(() =>
          runRepository(
            Effect.gen(function* () {
              const repository = yield* ProviderAssetRepository
              yield* repository.upsertProviderAssets({
        ...
        const reversalEvidenceIds = yield* Effect.promise(() =>
          runPg(
            Effect.gen(function* () {
              const db = yield* drizzle
              return (yield* db
    ...
    it.effect("rejects an on-chain exclusion reversal outside the revision-bound review flow", () =>
      Effect.gen(function* () {
        const providerAssetId = "btc-approval-manual-exclusion-reversal"
        const providerAsset = yield* Effect.promise(() =>
          seedPendingApprovalAsset("manual-exclusion-reversal")
    ...
    it.effect("accepts exact representation evidence observed after approval", () =>
      Effect.gen(function* () {
        const providerAsset = yield* Effect.promise(() =>
          seedPendingApprovalAsset("exact-approved-use", {
            withProviderTransfer: false,
          })
        )
        const observation = yield* Effect.promise(() => loadBitcoinObservation())
        yield* Effect.promise(() =>
          runRepository(
            Effect.flatMap(ProviderAssetRepository, (repository) =>
              repository.approveProviderAssetMappingAndRequestReplay({
    ...
    it.effect("accepts weaker evidence that does not contradict an approved representation", () =>
      Effect.gen(function* () {
        const providerAsset = yield* Effect.promise(() =>
          seedPendingApprovalAsset("partial-approved-use", {
            withProviderTransfer: false,
          })
        )
        const observation = yield* Effect.promise(() => loadBitcoinObservation())
    ...
    it.effect.each([
      ...
      ["decimals", { decimals: 9 }],
    ] as const)("rejects approved evidence with a mismatched %s", ([_field, override]) =>
      Effect.gen(function* () {
        const providerAsset = yield* Effect.promise(() =>
          seedPendingApprovalAsset(`mismatched-${_field}`, {
            withProviderTransfer: false,
          })
        )
        const observation = yield* Effect.promise(() => loadBitcoinObservation())
            ...
            Effect.result(
              Effect.flatMap(ProviderAssetRepository, (repository) =>
                repository.recordProviderAssetSourceUses({
                  ...
                  providerAssetRowIds: [providerAsset.id],
                  observations: [
                    { providerAssetRowId: providerAsset.id, ...observation, ...override },
                  ],
                })
              )
            )
    ...
    it.effect("rejects newly persisted evidence that conflicts with an approved mapping", () =>
      Effect.gen(function* () {
        const providerAsset = yield* Effect.promise(() =>
          seedPendingApprovalAsset("conflicting-approved-use", {
            withProviderTransfer: false,
          })
        )
        yield* Effect.promise(() =>
        ...
        const sourceUses = yield* Effect.promise(() =>
          runPg(
            Effect.gen(function* () {
              const db = yield* drizzle
              return yield* db
    ...
    it.effect(
      "rolls back normalized artifacts when approval wins before conflicting evidence persists",
      () =>
        Effect.gen(function* () {
          const providerAsset = yield* Effect.promise(() =>
            seedPendingApprovalAsset("approval-wins-persistence-race", {
              withProviderTransfer: false,
            })
          )
          yield* Effect.promise(() =>
          ...
          const conflictingBlockchainId = yield* Effect.promise(() =>
            runPg(
              Effect.gen(function* () {
                const db = yield* drizzle
                    ...
                    sourceNormalizationRepository.persistNormalizedArtifacts({
                      ...
                      transaction: {
                        ...
                        timestamp: DateTime.toDateUtc(
                          DateTime.makeUnsafe("2025-04-21T10:00:00.000Z")
                        ),
          ...
          const persistedState = yield* Effect.promise(() =>
            runPg(
              Effect.gen(function* () {
                const db = yield* drizzle
    ...
    it.effect(
      "keeps approval pending when conflicting normalization reserves the mapping first",
      () =>
        Effect.gen(function* () {
          const providerAsset = yield* Effect.promise(() =>
            seedPendingApprovalAsset("normalization-wins-persistence-race", {
              withProviderTransfer: false,
            })
          )
          const conflictingBlockchainId = yield* Effect.promise(() =>
          ...
          const sourceUseReserved = yield* Deferred.make<void>()
              ...
              yield* syncEngineTransaction.run(
                sourceNormalizationRepository.persistNormalizedArtifacts({
                  ...
                  transaction: {
                    ...
                    externalGroupId: null,
                    timestamp: DateTime.toDateUtc(DateTime.makeUnsafe("2025-04-21T11:00:00.000Z")),
                    transactionType: "internal_transfer",
                    providerTransactionType: "send",
                    providerStatus: "completed",
                    providerResourcePath: null,
                  ...
                  providerTransfers: [
                    {
                      ...
                      providerAssetId: providerAsset.id,
                      timestamp: DateTime.toDateUtc(
                        DateTime.makeUnsafe("2025-04-21T11:00:00.000Z")
                      ),
                      direction: "outbound",
                      processingMode: "evidence_only",
                      fromAccountRef: null,
          ...
          yield* Deferred.await(sourceUseReserved)
          const approval = runRepository(
            Effect.result(
              Effect.flatMap(ProviderAssetRepository, (repository) =>
                repository.approveProviderAssetMappingAndRequestReplay({
          ...
          const [, approvalResult] = yield* Effect.promise(() =>
            Promise.all([normalization, approval])
          )
          const state = yield* Effect.promise(() =>
            runPg(
              Effect.gen(function* () {
    ...
    it.effect.each(["pending", "processing"] as const)(
      ...
      (status) =>
        Effect.gen(function* () {
          const providerAsset = yield* Effect.promise(() =>
            seedPendingApprovalAsset(`approved-use-${status}`, {
              withProviderTransfer: false,
            })
          )
          yield* Effect.promise(() =>
          ...
          const [firstCount, secondCount] = yield* Effect.promise(() =>
            runRepository(
              Effect.flatMap(ProviderAssetRepository, (repository) =>
                Effect.all(
                  [
    ...
    it.effect("locks observation sources before provider assets", () =>
      Effect.gen(function* () {
        const providerAsset = yield* Effect.promise(() =>
          seedPendingApprovalAsset("source-first-lock")
        )
        const sourceLockAcquired = yield* Deferred.make<void>()
        const updateProviderAsset = yield* Deferred.make<void>()
                ...
                yield* tx
                  ...
                  .set({
                    retrievedAt: DateTime.toDateUtc(
                      DateTime.makeUnsafe("2026-08-16T12:00:00.000Z")
                    ),
                  })
                  .where(eq(schema.providerAssets.id, providerAsset.id))
              })
        ...
        yield* Deferred.succeed(updateProviderAsset, undefined)
        const [, result] = yield* Effect.promise(() =>
          Promise.all([heldNormalizationLock, approval])
        )
    ...
    it.effect("holds exclusion snapshot locks until the decision commits", () =>
      Effect.gen(function* () {
        const providerAsset = yield* Effect.promise(() =>
          seedPendingApprovalAsset("exclusion-holds-snapshot-locks")
        )
        const advisoryLockAcquired = yield* Deferred.make<void>()
        const releaseAdvisoryLock = yield* Deferred.make<void>()
        ...
        yield* Deferred.await(advisoryLockAcquired)
        yield* Effect.promise(() =>
          runPg(
            Effect.gen(function* () {
              const db = yield* drizzle
        ...
        yield* Deferred.succeed(releaseAdvisoryLock, undefined)
        const [, result] = yield* Effect.promise(() =>
          Promise.all([heldAdvisoryLock, exclusion, sourceLock])
        )
    ...
    it.effect("reattaches replay when another active owner wins the insert race", () =>
      Effect.gen(function* () {
        const providerAsset = yield* Effect.promise(() =>
          seedPendingApprovalAsset("insert-race-owner")
        )
        yield* Effect.promise(() =>
          runPg(
            Effect.gen(function* () {
    ...
    it.effect("reattaches replay when a source-use insert loses the active-owner race", () =>
      Effect.gen(function* () {
        const providerAsset = yield* Effect.promise(() =>
          seedPendingApprovalAsset("source-use-insert-race-owner", {
            withProviderTransfer: false,
          })
        )
        yield* Effect.promise(() =>
    ...
    it.effect(
      "falls back to provider-scoped natural-key lookup when provider asset id is absent",
      () =>
        Effect.gen(function* () {
          const upsertCount = yield* Effect.promise(() =>
            runRepository(
              Effect.flatMap(ProviderAssetRepository, (repository) =>
                repository.upsertProviderAssets({
                  providerKey: "coinbase",
    ...
    it.effect("rejects a network representation that belongs to another economic asset", () =>
      Effect.gen(function* () {
        yield* Effect.promise(() =>
          runRepository(
            Effect.flatMap(ProviderAssetRepository, (repository) =>
    ...
    it.effect("pages provider asset reviews with a stable provider asset cursor", () =>
      Effect.gen(function* () {
        yield* Effect.promise(() =>
          runRepository(
            Effect.flatMap(ProviderAssetRepository, (repository) =>
        ...
        const providerAssets = yield* Effect.promise(() =>
          runRepository(
            Effect.gen(function* () {
              const repository = yield* ProviderAssetRepository
    ...
    it.effect(
      "keeps reviewed natural-key mappings preferred when a stable provider asset id arrives later",
      () =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            runRepository(
          ...
          const naturalKeyProviderAsset = yield* Effect.promise(() =>
            runRepository(
              Effect.flatMap(ProviderAssetRepository, (repository) =>
                repository.findProviderAssetByNaturalKey({
                  providerKey: "coinbase",
          ...
          const resolvedProviderAsset = yield* Effect.promise(() =>
            runRepository(
              Effect.flatMap(ProviderAssetRepository, (repository) =>
    ...
    it.effect("keeps excluded natural-key mappings preferred over newer unmapped stable ids", () =>
      Effect.gen(function* () {
        yield* Effect.promise(() =>
          runRepository(
            Effect.flatMap(ProviderAssetRepository, (repository) =>
    ...
    it.effect("fails when a provider asset entry has neither stable id nor natural key", () =>
      Effect.gen(function* () {
        const error = yield* Effect.promise(() =>
          runRepository(
            Effect.flatMap(ProviderAssetRepository, (repository) =>
  ...
  describe("decision history", () => {
    ...
    it.effect(
      "re-recording at the same evidence revision reports recorded: false and keeps the original",
      () =>
        Effect.gen(function* () {
          const { providerAssetRowId } = yield* Effect.promise(() =>
            scheduleResolutionJob("history-duplicate")
          )
    ...
    it.effect("rejects stale job evaluations without persisting decision or evidence history", () =>
      Effect.gen(function* () {
        const { providerAssetRowId } = yield* Effect.promise(() =>
          scheduleResolutionJob("history-stale-job")
        )
        yield* Effect.promise(() =>
          runPg(
            Effect.gen(function* () {
                  ...
                  evidence: [
                    {
                      ...
                      retrievedAt: DateTime.toDateUtc(
                        DateTime.makeUnsafe("2026-08-26T20:00:00.000Z")
                      ),
                      evidenceRevision: 1,
    ...
    it.effect("preserves legacy automatic supersession rows with the same evaluation key", () =>
      Effect.gen(function* () {
        const { providerAssetRowId } = yield* Effect.promise(() =>
          scheduleResolutionJob("history-legacy-supersession")
        )
    ...
    it.effect("rejects supersession links that cross observation histories", () =>
      Effect.gen(function* () {
        const { providerAssetRowId: firstObservationId } = yield* Effect.promise(() =>
          scheduleResolutionJob("history-cross-first")
        )
        const { providerAssetRowId: secondObservationId } = yield* Effect.promise(() =>
          scheduleResolutionJob("history-cross-second")
        )

        const crossObservation = yield* Effect.promise(() =>
          runPg(
            Effect.gen(function* () {
              const db = yield* drizzle
    ...
    it.effect(
      "establishes the first settled policy result as the conclusion without replacing one later",
      () =>
        Effect.gen(function* () {
          const { providerAssetRowId } = yield* Effect.promise(() =>
            scheduleResolutionJob("history-first-conclusion")
          )
          ...
          const [advancedState] = yield* Effect.promise(() =>
            runPg(
              Effect.gen(function* () {
                const db = yield* drizzle
                return yield* db
          ...
          expect(advancedState?.currentPolicyEvaluationId).not.toBe(
            state?.currentPolicyEvaluationId
          )
        })
    )

    it.effect("keeps a late older evaluation from regressing either current pointer", () =>
      Effect.gen(function* () {
        const { providerAssetRowId } = yield* Effect.promise(() =>
          scheduleResolutionJob("history-late-evaluation")
        )
        yield* Effect.promise(() =>
          runPg(
            Effect.gen(function* () {
    ...
    it.effect("appends a new policy revision without changing the prior evaluation", () =>
      Effect.gen(function* () {
        const { providerAssetRowId } = yield* Effect.promise(() =>
          scheduleResolutionJob("history-supersede")
        )
    ...
    it.effect(
      "stores authority-scoped evidence records behind a decision and reads them back",
      () =>
        Effect.gen(function* () {
          const { providerAssetRowId } = yield* Effect.promise(() =>
            scheduleResolutionJob("history-evidence")
          )
          const retrievedAt = DateTime.toDateUtc(DateTime.makeUnsafe("2025-06-01T12:00:00.000Z"))
    ...
    it.effect(
      "rejects a second evidence record for the same authority and claim kind on one decision",
      () =>
        Effect.gen(function* () {
          const { providerAssetRowId } = yield* Effect.promise(() =>
            scheduleResolutionJob("history-evidence-duplicate")
          )
          const retrievedAt = yield* DateTime.nowAsDate
          ...
          yield* Effect.promise(() => expect(duplicateInsert).rejects.toThrow())
        })
    )

    it.effect(
      "lets the database reject a second active decision for one observation and revision",
      () =>
        Effect.gen(function* () {
          const { providerAssetRowId } = yield* Effect.promise(() =>
            scheduleResolutionJob("history-db-guard")
          )
          ...
          yield* Effect.promise(() => expect(directInsert).rejects.toThrow())
        })
    )
  })
})
