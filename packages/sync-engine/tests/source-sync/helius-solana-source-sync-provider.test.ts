...
describe("HeliusSolanaSourceSyncProviderLive", () => {
  it("records source use before returning a prepared record", async () => {
    ...
    expect(sourceUses).toEqual([
      {
        sourceId: "source-solana-1",
        providerAssetRowIds: ["provider-asset-sol"],
      },
    ])
  })
  ...
  it("rejects exact wallet-row decimals that conflict with an approved mapping", async () => {
    const walletTransferEvidence = [
      {
