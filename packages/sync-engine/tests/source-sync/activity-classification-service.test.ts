...
describe("ActivityClassificationService", () => {
  it.effect("returns the deterministic review-required fallback for unknown facts", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        expect(classify(facts)).resolves.toEqual(
  ...
  it.effect("preserves evidence when constructing a fallback classification", () =>
    Effect.gen(function* () {
      const facts = yield* Schema.decodeEffect(ActivityFacts)({
        sourceKind: "solana",
        providerKey: "helius-solana",
        sourceId: "source-solana-1",
        externalId: "5mSolanaFixtureSignature",
        occurredAt: DateTime.toDateUtc(DateTime.makeUnsafe("2026-01-02T00:00:00.000Z")),
      ...
      const result = yield* Effect.promise(() => classify(facts))

      expect(result.evidence).toEqual(solanaEvidence)
    })
  )

  it.effect("accepts Solana-shaped activity facts without Solana-only result fields", () =>
    Effect.gen(function* () {
      const facts = yield* Schema.decodeEffect(ActivityFacts)({
        sourceKind: "solana",
        providerKey: "helius-solana",
        ...
        occurredAt: DateTime.toDateUtc(DateTime.makeUnsafe("2026-01-03T00:00:00.000Z")),
  ...
  it.effect("accepts CEX-shaped activity facts without provider-specific result fields", () =>
    Effect.gen(function* () {
      const facts = yield* Schema.decodeEffect(ActivityFacts)({
        sourceKind: "cex",
        ...
        occurredAt: DateTime.toDateUtc(DateTime.makeUnsafe("2026-01-04T00:00:00.000Z")),
