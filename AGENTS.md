...
One narrow exception, recorded on #149 for T10c: when a migration adds a required column that records a fact only the writer can supply, to a table that is derived state fully regenerated by source replay, the table may be cleared inside that migration. Conditions: Max explicitly approves that one statement on the spec issue, the migration carries an inline SQL comment with the rationale, and the PR is incomplete until replay has repopulated the table and a regression proves the writer supplied the new column. A default value or a nullable staging column is not an alternative; both record false facts (ADR 0012).
