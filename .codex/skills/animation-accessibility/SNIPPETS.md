...
Copy-ready recipes for the cases that need a specific mechanism rather than an easing swap. From the _Animations on the Web_ course ([animations.dev](https://animations.dev/)).

## Smooth scrolling

Scroll-behavior is motion the user didn't ask for, so opt _in_ under `no-preference` rather than opting out under `reduce`. Written this way, the accessible behavior is the default even in browsers that don't match either query:
...
<picture>
  ...
  <source
    ...
    media="(prefers-reduced-motion: no-preference)"
  />
  <source srcset="animated.gif" type="image/gif" media="(prefers-reduced-motion: no-preference)" />
  <!-- Static fallback -->
...
Because the first render is always `false`, don't let the value gate _mounting_ — branch animation values only, or the reduced-motion user sees a flash of the animated variant.
