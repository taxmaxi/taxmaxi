...
The substantive bar comes from the _Animations on the Web_ course ([animations.dev](https://animations.dev/)) — every rule below traces to a lesson. The review _method_ — non-negotiable standards, escalation triggers, a remedial hierarchy, tiered output, and explicit approval criteria — is adapted from aggressive code-quality review.
...
- Everything-at-once entrance where a 30–80ms stagger belongs; **or** a parent entrance _plus_ staggered children (double entry — one entrance per container)
