...
The bar comes from the _Animations on the Web_ course ([animations.dev](https://animations.dev/)). The full rule catalog with exact values lives in [AUDIT.md](AUDIT.md); the plan format lives in [PLAN-TEMPLATE.md](PLAN-TEMPLATE.md). Load each when you reach the phase that needs it.
