...
The CSS Animations module of Emil Kowalski's _Animations on the Web_ course ([animations.dev](https://animations.dev/)), as a working reference. Everything here is CSS only — no dependencies, no JavaScript.
...
- **`animation-fill-mode: backwards`** applies the _first_ keyframe before the animation starts — the fix for a delayed enter animation flashing its natural state first. `both` does both.
...
And when hover reveals _information_ rather than decoration, pair it with **`:focus-visible`** so keyboard users get it too. (`:focus-visible` fires for keyboard focus; `:focus` also fires on click.)
