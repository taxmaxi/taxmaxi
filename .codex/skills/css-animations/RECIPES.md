...
```tsx
{
  new Array(LENGTH)
    .fill(0)
    .map((_, i) => <div className="card" key={i} style={{ "--index": LENGTH - 1 - i }} />)
}
```
...
.h1 {
  overflow: hidden;
} /* hides the letters' parked position */
...
@keyframes orbit {
  from {
    transform: translate(-50%, -50%) rotateY(0deg) translateZ(74px) rotateY(360deg);
  }
  to {
    transform: translate(-50%, -50%) rotateY(360deg) translateZ(74px) rotateY(0deg);
  }
}
```
