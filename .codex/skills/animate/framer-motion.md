...
Recipes for `motion/react` (formerly `framer-motion` — same API, only the import path changed). Load from [SKILL.md](SKILL.md) when building JS-driven or interruptible motion. All values from the _Animations on the Web_ course.
...
- Give two elements the same **`layoutId`** to morph one into the other across mount/unmount — tab indicators, App-Store card→detail, button→popover, the trash "throw." You can't steer _how_ it moves; to add extra motion, animate the **parent** so children follow (`<motion.div animate={{ y: 73 }} transition={{ delay: 0.13 }}>`).
...
;<motion.div animate={{ height: bounds.height ? bounds.height : null }}>
  {" "}
  {/* null → auto on 1st render, avoids shift */}
  <div ref={ref} className="p-6">
    {content}
  </div>{" "}
  {/* ref on an INNER element that carries the padding */}
</motion.div>
```
